...
# 지구 거시 왜곡 지수 (S_earth)[cite: 1, 4]
S_EARTH = PI_SQ / G_STANDARD 
# 절대 광속 확정 (c_k)[cite: 1, 3]
C_K = C_SI / S_EARTH

st.set_page_config(page_title="K-PROTOCOL 3-Layer Engine", layout="wide")
st.title("🛡️ KR K-PROTOCOL: 3-Layer Unification Engine")
st.markdown("현대 물리학의 **국소적 착시(System E)**를 벗겨내고, **우주 절대 기하학(System U)**으로 변환한 뒤, 이를 다시 **관측자의 시계(Observation)**로 투영하는 3단계 증명 시스템입니다.[cite: 1]")
st.write("---")
...
st.error("**문제점**: 현대 물리학은 지구 중력에 의해 '수축된 고도($h$)'와 '왜곡된 광속($c$)'을 고정 상수로 사용하는 순환 오류에 빠져 있습니다.[cite: 1]")
...
with col1_1:
    ...
    # 파이썬 f-string 충돌 방지를 위해 이중 중괄호 {{ }} 사용
...
st.success("**해결책**: K-PROTOCOL은 삐뚠 광속과 삐뚠 고도를 텐서($S_{loc}$)로 일제히 펴서, 왜곡 0%의 '우주 절대 기하학' 스케일로 동기화합니다.[cite: 1, 3]")
...
st.info("**증명**: 우주의 절대 시간(5.011e-14)을 다시 인간의 '왜곡된 원자시계 렌즈($S_{avg}^2$)'로 투영하면, 기하학적 굴절을 거쳐 도쿄대 관측치 근방으로 정확히 수렴합니다.[cite: 1]")
...
현대 물리학이 측정한 `4.930`은 우주의 절대 시간이 아닙니다. 우주의 진짜 시간 지연은 **Layer 2의 `5.011`**이며, 우리가 지구라는 찌그러진 공간($S_{avg}^2$) 안에서 시계를 보기 때문에 빛이 굴절되어 **Layer 3의 `4.948`**로 보이는 것뿐입니다. (남은 잔차는 조석력 등 순수 환경 변수입니다.)[cite: 1, 10]
""")
